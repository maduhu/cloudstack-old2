...
CREATE VIEW `cloud`.`template_view` AS
    ...
    from
        ...
        `cloud`.`vm_template` source_template ON source_template.id = vm_template.source_template_id    
            left join
        `cloud`.`template_store_ref` ON template_store_ref.template_id = vm_template.id and ((template_store_ref.store_role = 'Image')  or (vm_template.hypervisor_type = 'BareMetal'))
            left join
